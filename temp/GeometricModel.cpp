...
bool GeometricModel::learnPattern(bool bShowImage) {
	...
	// duyệt qua từng contours trong template và lần lượt đẩy data của từng điểm trong contours vào struct patternInfo
	patternInfo.clear();
	for (int contourIdx = 0; contourIdx < contours.size(); contourIdx++) {
		for (int pointInx = 0; pointInx < contours[contourIdx].size(); pointInx++)
		{
			...
			double mag;
			tempData.Coordinates = contours[contourIdx][pointInx];   // vị trí
			tempData.Derivative = Point2d(gx.at<double>(tempData.Coordinates) // derivarative(gx,gy)
				, gy.at<double>(tempData.Coordinates));
			tempData.Angle = angle.at<double>(tempData.Coordinates);// angel
			mag = magnitude.at<double>(tempData.Coordinates);// magnitude
	...
	// find contour có diện tích lớn nhất trong teamplate
	int maxNumPoint = 0;
	...
	// xử lý pca cho contours được chọn
...
void GeometricModel::getPcaOrientation(const std::vector<Point>& pts, double& angleOutput, Point2f& centerOutput) {
	...
	// tính tâm của dữ liệu 
	Point cntr = Point(static_cast<int>(pca_analysis.mean.at<double>(0, 0)),
		static_cast<int>(pca_analysis.mean.at<double>(0, 1)));
	//tính góc và tâm pca bằng eigenvalues and eigenvectors
