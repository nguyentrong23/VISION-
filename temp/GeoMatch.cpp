...
void GeoMatch::Matching(void)
{
	...
	//Canny(imageGray, imageThreshold, cannyThresh_1, cannyThresh_2,cannyKernelSize);// có hoặc không canny đều được
	findContours(imageThreshold, srcContours, srcHierarchy,cv::RETR_TREE, cv::CHAIN_APPROX_NONE);
	// duyệt qua từng biên của source
	for (int conCounter = 0; conCounter < srcContours.size(); conCounter++) {
		ObjectInfo tempObject;
		//tính diện tích contours của source
		...
		for (int listCounter = 0; listCounter < ModelSrc.size(); listCounter++) {
			...
			ModelSrc[listCounter].getThreshArea(lowThresh, highThresh);// lấy ngưỡng diện tích của mỗi contours trong model
			// nếu diện tích của contours nằm trong ngưỡng của model thì đưa vào check list của object tạm
			if ((area >= lowThresh) && (area <= highThresh)) { 
				tempObject.modelCheckList.push_back(listCounter);
			}
		}
		// nếu danh sách check list không k rỗng /////////////////////////////////////////////////////////
		if (tempObject.modelCheckList.size() > 0) {
			tempObject.conArea = area;      // gáng diện tích của countors của source 
			tempObject.conIndex = conCounter;  // vị trí của contours là thứ mấy trong srcContours
			tempObject.conBoundingRect = boundingRect(srcContours[conCounter]);  // lấy hình chữ nhật
			...
			objects.push_back(tempObject); // đẩy tempObject vào list objects
		}
	}
	//////////////////////////////////////////////////////////
	vector<MatchedObjects> matchedList;   // MatchedObjects là struct có coordi, Mat và angel
...
void GeoMatch::getRotatedROI(Mat& matSr, GeometricModel& model, ObjectInfo& object, GetROI_MODE roiMode) {
...
void GeoMatch::getRotatedROI(Mat& matSrc, GetROI_MODE roiMode, GeometricModel& model, ObjectInfo& object) {
...
bool GeoMatch::matchingScores(RotatedObject& objectRotated, GeometricModel& model, double& lastMaxScores) {
...
bool GeoMatch::matchingScores(Mat& inputImage, GeometricModel& model, double& lastMaxScores) {
	// add check pattern learn and image empty
